<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:s="http://syriaca.org" xmlns:t="http://www.tei-c.org/ns/1.0" xmlns:x="http://www.w3.org/1999/xhtml" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:saxon="http://saxon.sf.net/" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:local="http://syriaca.org/ns" exclude-result-prefixes="xs t s saxon" version="2.0">
    ...
    <xsl:template name="spearCite">
        <div class="citation">
            ...
            <span class="element">
                ...
                <xsl:if test="ancestor-or-self::t:TEI/descendant::t:seriesStmt/t:title[@level='m']">
                    <xsl:text>in </xsl:text><xsl:apply-templates select="ancestor-or-self::t:TEI/descendant::t:seriesStmt/t:title[@level='m']"/>: <xsl:apply-templates select="ancestor-or-self::t:TEI/descendant::t:seriesStmt/t:title[@type='sub']"/>
                    <xsl:text>, </xsl:text></xsl:if>
                <xsl:if test="ancestor-or-self::t:TEI/descendant::t:seriesStmt[t:title[@level='m']]/t:editor">
                    <xsl:text>edited by </xsl:text><xsl:apply-templates select="ancestor-or-self::t:TEI/descendant::t:seriesStmt[t:title[@level='m']]/t:editor"/><xsl:text>, </xsl:text></xsl:if>
    ...
    <xsl:template name="refs">
        <xsl:variable name="refIndexs" select="$keywordsIndex | $biblIndex | $placesIndex | $personsIndex"/>
        <xsl:for-each-group select="descendant-or-self::*/@ref | descendant-or-self::*/@ana" group-by="tokenize(.,'/')[4]">
            <xsl:if test="contains(current-grouping-key(),'person') or contains(current-grouping-key(),'place') or contains(current-grouping-key(),'taxonomy')">
                ...
                <ul>
                        ...
                        <xsl:for-each select="distinct-values($refs/tokenize(.,' '))">
                            ...
                            <xsl:variable name="titleString">
                                <xsl:choose>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:value-of select="replace(concat(upper-case(substring(tokenize(.,'/')[last()],1,1)),substring(tokenize(.,'/')[last()],2)),'_',' ')"/>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:variable>
                            <xsl:variable name="url">
                                <xsl:value-of select="concat('/aggregate/',tokenize(.,'/')[4],'/',tokenize(.,'/')[last()],'.html')"/>
                            </xsl:variable>
                            <xsl:if test="$titleString != ''">
                                <li>
                                    <a href="{$url}">
                    ...
                    <xsl:for-each select="$relatedList/child::*">
                        <xsl:sort select="descendant-or-self::text()"/>
                        <xsl:if test="normalize-space(string-join(descendant-or-self::text(),'')) != ''">
                            <xsl:copy-of select="."/>
                        </xsl:if>
                    </xsl:for-each>
                </ul>
            ...
            <xsl:if test="contains(current-grouping-key(),'person') or contains(current-grouping-key(),'place') or contains(current-grouping-key(),'taxonomy')">
                ...
                <ul>
                    <xsl:for-each select="distinct-values($refs/tokenize(.,' '))">
                        ...
                        <li>
                            <a href="{$url}"><span class="loadRDF getLabel" data-rdfRef="{$ref}"><xsl:value-of select="local:resolveURI($ref)"/></span></a>
                            <!--
                            <xsl:value-of select="$ref"/> ||
                            <xsl:choose>
                                <xsl:when test="$personsIndex/child::* != ''">
                                    <xsl:value-of select="$personsIndex/descendant::*:person[*:idno = $ref]/descendant::*:title/descendant-or-self::text()"/>
                                </xsl:when>
                                <xsl:otherwise><xsl:message>problem</xsl:message></xsl:otherwise>
                            </xsl:choose>
                            -->
    ...
    <xsl:template match="t:keyword-title">
        <div class="row title padding-top">
            <h1 class="col-md-8">
                ...
                <xsl:value-of select="substring-after(//tei:idno, '/taxonomy/')"/>
            </h1>
        </div>
        <div style="margin:0 1em 1em; color: #999999;">
            <small>
    ...
    <xsl:template match="t:aggregate">
        ...
        <xsl:variable name="title">
            <xsl:choose>
                ...
                <xsl:when test="contains($id,'/taxonomy')"><xsl:value-of select="replace(tokenize($id,'/')[last()],'_',' ')"/></xsl:when>
            </xsl:choose>
        </xsl:variable>
                    ...
                    <span class="element">
                        <xsl:choose>
                            <xsl:when test="contains($id,'/person/')">
                                ...
                                <xsl:value-of select="$url"/>, <xsl:value-of select="current-date()"/>.
                            </xsl:when>
                            <xsl:when test="contains($id,'/place/')">
    ...
    <xsl:template match="t:trait | t:occupation | t:socecStatus" mode="spear">
        <xsl:choose>
            ...
            <xsl:when test="@ana">
                <xsl:value-of select="substring-after(@ana,'/taxonomy/')"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:apply-templates mode="spear"/>
            </xsl:otherwise>
