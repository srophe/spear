...
<!--                                    <xsl:message>Bibl item not found. the value of biblfilepath is ' <xsl:value-of select="$newBiblfilepath"/>'</xsl:message>-->
                ...
                <xsl:when test="contains(descendant::t:ptr/@target,'/work')">
                    <!-- place holder -->
                </xsl:when>
                <xsl:when test="$biblIndex">
                    <xsl:variable name="target" select="descendant::t:ptr/@target"/>
                    <xsl:variable name="citation">
                        <xsl:sequence select="$biblIndex/descendant::*:bibl[*:idno = $target]"/>    
                    </xsl:variable>
                    <xsl:apply-templates select="$citation/descendant::*:bibl[@subtype='citation']" mode="formattedCitation"/>
                    <xsl:sequence select="$passThrough"/>
                    <xsl:if test="$citation/descendant::t:idno[@type='URI']">
                        <span class="footnote-links">
                            <xsl:apply-templates select="$citation/descendant::t:idno[@type='URI']" mode="links"/>
                            <xsl:apply-templates select="$citation/descendant::t:ref[not(ancestor::note)]" mode="links"/>
                        </span>
                    </xsl:if>
                </xsl:when>
                <xsl:otherwise>
