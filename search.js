...
// BUILD THE SQL QUERIES CUSTOM, MULTIFIELD Union, but ethnicity broken
...
//   const genderPattern = Array.from(state.selectedGenderKeywords || []).map(uri => `
//     {
//       ?person swdt:gender <${uri}> .
//     }
//     `);



//   const uncertaintyFilter = Array.from(state.selectedUncertaintyKeywords || []).map(selectedLevel => `
//       FILTER NOT EXISTS {
//         ?factoid spq:certainty ?level .
//         FILTER(LCASE(STR(?level)) = "${selectedLevel.toLowerCase()}")
//       }
//     `
//     ).join('\n');
...
// SELECT DISTINCT ?factoid ?description ?person ?label ?gender ?relationship ?place ?ethnicity ?
// FROM <http://syriaca.org/persons#graph>
// FROM <https://spear-prosop.org>
// WHERE {
